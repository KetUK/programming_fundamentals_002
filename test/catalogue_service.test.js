...
describe("catalogueService", () => {
  ...
  describe("catalogueService.countBooksByKeyword", () => {
    test("returns 2 the number of books for given Keyword normal", () => {
      expect(catalogueService.countBooksByKeyword("normal")).toBe(2);
    });

    test("returns 3 the number of books for given Keyword assassin", () => {
      expect(catalogueService.countBooksByKeyword("assassin")).toBe(3);
    });

    test("returns 3 the number of books for given Keyword pineapple", () => {
      expect(catalogueService.countBooksByKeyword("pineapple")).toBe(0);
    });
  });

  describe("catalogueService.getBooksByAuthor", () => {
    test("returns list of books for given Author Charles Dickens", () => {
    ...
    test("returns empty list of books for given Author Ketaki", () => {
      expect(catalogueService.getBooksByAuthor("Ketaki")).toEqual([]);
    });
  });
